...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const mathSqrt1 = Math.sqrt(x1 * x1 + y1 * y1);
  const mathSqrt2 = Math.sqrt(x2 * x2 + y2 * y2);
  const cos = (x1 * x2 + y1 * y2) / (mathSqrt1 * mathSqrt2);
...
function toNumber(value, def) {
  ...
  if (Number.isNaN(toNum)) return def;
  return toNum;
}
...
function getSumOfDigits(num) {
  ...
  let result = 0;
  for (let i = 0; i <= numToStr.length - 1; i += 1) {
    result += toNumber(numToStr[i]);
  }
  return result;
}
...
function isPowerOfTwo(num) {
  if ((Math.log(num) / Math.log(2)) % 1 === 0) return true;
  return false;
}
